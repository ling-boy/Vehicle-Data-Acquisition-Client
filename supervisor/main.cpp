...
int main(int argc, char* argv[]) {
    ...
    try {
        std::string mqNameCollect = std::string(MQ_CONTROL_FMT);
        // 使用简单名称
    ...
    // 7. 注册信号回调
    SignalHandler::instance().onSignal(SIGTERM, [](int sig) {
        LOG_INFO("Received SIGTERM, initiating shutdown...");
    });
    SignalHandler::instance().onSignal(SIGINT, [](int sig) {
    ...
    while (!SignalHandler::instance().shouldExit()) {
        // 检查信号（非阻塞）
        SignalHandler::instance().waitForSignal(500);
